...
void MainWindow::simulation(){
    ...
    if(save){  //si guardar está activado, guarda la posicion del cuerpo en un archivo txt
...
void MainWindow::get_export_values() {  //escribe los velores en un txt
...
void MainWindow::on_start_button_clicked() {
    ...
    timer->stop(); //no se ejecuta

    if(!ui->resume_button->isEnabled()) //habilitar botones
    ...
    if(file.isOpen()){ //Guarda si está abierto
        file.flush();
        file.close();
        ui->save_label->setText("Guardado exitosamente");
    }
    else
...
void MainWindow::on_Export_button_clicked() {
    ...
    file.setFileName(file_name); //crea el archivo
    ...
    get_export_values(); //Recopila los valores

    file.flush();
    file.close();

    ui->save_label->setText("Exportado correctamente");
}
void MainWindow::on_Import_button_clicked() {
    ...
    std::fstream file (file_name.toStdString(), std:: fstream::in | std::fstream::binary);
     ...
     if(file.is_open()){
         ...
         update_boxes(); //actualizar datos importados

         ui->save_label->setText("Importado correctamente");
     }
     else {
         ui->save_label->setText("Error importando");
     }
}
